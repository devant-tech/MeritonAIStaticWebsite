...
for (let i = 0; i < 10; i++) {
    comb.push([]);
    for (let j = 0; j < 10; j++) {
        comb[i].push(combinations(i, j));
    }
}

const bernstein = (n: number, k: number, x: number) =>
    comb[n][k] * Math.pow(1 - x, n - k) * Math.pow(x, k);
...
export class ControlPoints {
    ...
    index(i: number, j: number, k: number) {
        return this.points[
            i * this._length * this._height + j * this._height + k
        ];
    }
...
export const parametricToCartesian = (
    point: THREE.Vector3,
        ...
        for (let j = 0; j <= m; j++) {
            for (let k = 0; k <= n; k++) {
                const weight =
                    ...
                    bernstein(m, j, t) *
                    bernstein(n, k, u);
                displacedPoint.addScaledVector(
                    controlPoints.index(i, j, k),
                    weight
                );
            }
        }
...
class FFD {
    ...
    controlPoints: ControlPoints;
    private _position: THREE.BufferAttribute;
    ...
    constructor(
        ...
        this._position = cartesianToParametric(position, coord);
    }
    ...
    deform() {
        ...
        for (let i = 0; i < this._position.count; i++) {
            _stu.fromBufferAttribute(this._position, i);
